...
#make plot
p1 <- ggplot() +
  geom_sf(data=plot_data, aes(fill=as.numeric(gsub(",","",d$`All Ages`))), colour="grey90", size=0.5) +
  ...
  theme(plot.background = element_rect(fill = "grey90", colour="grey90"),
        ...
        plot.title = element_text(colour = "#c90055", size=24, hjust = 0, family="serif", face="bold"),
        plot.subtitle = element_text(colour = "#c90055", size=9, hjust = 0, family="serif"),
        plot.caption = element_text(colour = "#c90055", size=9, hjust = 0, family="serif"),
        legend.title=element_text(colour = "#c90055", size=9, hjust = 0.5, family="serif"),
        legend.background = element_rect(fill = "transparent", colour="transparent"),
        legend.text = element_text(colour = "#c90055", size=9, hjust = 0.5, family="serif"),
...
#fix background
p <- ggdraw(p1) + 
  draw_label(x=0.98, y=0.75, hjust=1, 
             ...
             color = "#c90055", size = 9, fontfamily="serif") +
  theme(panel.background = element_rect(fill = "grey90", colour = "grey90"))
p

#save 3d image
plot_gg(p,theta=0, phi=70, width=5, height=5, zoom=.6, windowsize = c(600, 580))
render_snapshot("2021/viz/map_11", clear=TRUE)
